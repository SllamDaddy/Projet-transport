...
export default function LignesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Map stations and generate simple IDs
      ...
      <main className="flex-1 flex flex-col overflow-y-auto px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-sm text-dark-on-surface-variant">
            Configurez les lignes et les coordonnées GPS des gares pour le guidage conducteur
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1">
          {/* Liste des lignes */}
          <div className="lg:col-span-7 bg-dark-surface border border-dark-outline rounded-3xl p-6 shadow-md h-fit">
            <h3 className="text-lg font-bold text-dark-on-surface mb-6">Lignes actives</h3>
            ...
            {loading ? (
              ...
              <div className="text-center py-12 text-sm text-dark-on-surface-variant">
                Aucune ligne de bus configurée. Remplissez le formulaire pour en ajouter une.
              </div>
            ) : (
              <div className="space-y-4">
                {lignes.map((ligne) => (
                  <div key={ligne.id} className="border border-dark-outline/40 bg-dark-bg/25 rounded-2xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3">
                        <h4 className="font-bold text-dark-on-surface text-base">{ligne.nom}</h4>
                      ...
                      <p className="text-xs text-dark-on-surface-variant mt-1.5 font-medium">
                        {ligne.stations.length} arrêt(s) configuré(s) :
                      </p>
                      <div className="flex flex-wrap items-center gap-2 mt-2">
                        {ligne.stations.map((station, idx) => (
                          <React.Fragment key={station.id}>
                            {idx > 0 && <span className="text-xs text-slate-600">➔</span>}
                            <span className="bg-dark-surface-variant/50 px-2.5 py-1 rounded-lg text-xs font-semibold text-dark-on-surface-variant border border-dark-outline/30">
                              {station.name}
                            </span>
                          </React.Fragment>
                        ))}
                    ...
                    <div className="flex items-center gap-2 self-end md:self-center">
                      <button
                        ...
                        title={ligne.actif ? 'Désactiver la ligne' : 'Activer la ligne'}
                      >
          ...
          <div className="lg:col-span-5 bg-dark-surface border border-dark-outline rounded-3xl p-6 shadow-md h-fit">
            <h3 className="text-lg font-bold text-dark-on-surface mb-6">
              {editingLigne ? 'Modifier la ligne de bus' : 'Ajouter une ligne de bus'}
            </h3>

            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-dark-on-surface-variant">
                  Nom de la ligne
                </label>
                <input
                  type="text"
                  ...
                  {stations.map((station, index) => (
                    <div key={index} className="border border-dark-outline/30 bg-dark-bg/40 rounded-xl p-4 space-y-3 relative">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[10px] text-dark-on-surface-variant">Latitude</label>
                          <input
                            type="number"
                            step="any"
                            required
                        ...
                        <div>
                          <label className="block text-[10px] text-dark-on-surface-variant">Longitude</label>
                          <input
                            type="number"
                            step="any"
                            required
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-[10px] text-dark-on-surface-variant">Approche (m)</label>
                          <input
                            type="number"
                            required
                        ...
                        <div>
                          <label className="block text-[10px] text-dark-on-surface-variant">Arrêt (m)</label>
                          <input
                            type="number"
                            required
